...
PROBE_URL="${3:-/__api__/me}"

sign () {
  PROBE_URL="${3:-/__api__/me}"
  export TOKEN=$(cat token.txt)
  echo $(date -u "+%a, %d %b %Y %H:%M:%S GMT") > date.txt
  export DATE_PREP=$(cat date.txt)
  export DATE=${DATE:-$DATE_PREP}
  echo -n "GET
${HOST_BASE_PATH}${PROBE_URL}
${DATE}
?" > canonical.txt
...
if [ -z "$1" ]; then
  echo "Must pass 'gen' 'probe' 'req' or 'sign' as the first argument"
...
if [[ $1 == "req" ]]; then
  ...
  export SIGNATURE=$(cat signature64.txt)
  # Did it work?
  curl -i "$HOST$PROBE_URL" -H "Date: $DATE" -H "X-Auth-Token: $TOKEN" -H "X-Auth-Signature: $SIGNATURE" -H "X-Content-Checksum: ?"
fi
